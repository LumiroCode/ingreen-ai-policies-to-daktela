...
test('Daktela ticket policy data mapper maps non-empty policy values to custom fields', function (): void {
    $data = ExtractedPolicyData::fromFields([
        ...
        'wartosc_pojazdu_brutto' => '123 456,78 PLN',
...
test('Daktela module updates ticket policy data through form-encoded custom fields', function (): void {
    ...
    assertSameValue('12000', $body['customFields']['przebieg']);
    assertSameValue('1798', $body['customFields']['pojemnosc_silnika']);
    assertSameValue('123456.78', $body['customFields']['wartosc_pojazdu_brutto']);
...
test('Daktela module finds policy CRM record identifiers by registration number or VIN', function (): void {
    ...
    $recordIdentifiers = $module->findPolicyCrmRecordIdentifiers('ABC/123', 'WX12345', 'T M B 123');
    parse_str(parse_url($fake->requests[0]['url'], PHP_URL_QUERY) ?: '', $query);
...
test('Daktela module finds vehicle CRM record identifiers by registration number or VIN', function (): void {
    ...
    $recordIdentifiers = $module->findVehicleCrmRecordIdentifiers('ABC/123', 'WX12345', 'T M B 123');
    parse_str(parse_url($fake->requests[0]['url'], PHP_URL_QUERY) ?: '', $query);
...
test('Daktela module returns empty policy CRM record identifier list when nothing matches', function (): void {
    ...
    $recordIdentifiers = $module->findPolicyCrmRecordIdentifiers('123', 'WX12345', 'T M B 123');
...
test('Daktela module returns empty vehicle CRM record identifier list when nothing matches', function (): void {
    ...
    $recordIdentifiers = $module->findVehicleCrmRecordIdentifiers('123', 'WX12345', 'T M B 123');
...
test('Daktela module rejects matched policy CRM record without writable identifier', function (): void {
    ...
    try {
        $module->findPolicyCrmRecordIdentifiers('123', 'WX12345', 'T M B 123');
    } catch (\Ingreen\DaktelaPolicy\Support\AppException $exception) {
...
test('Daktela module rejects matched vehicle CRM record without writable identifier', function (): void {
    ...
    try {
        $module->findVehicleCrmRecordIdentifiers('123', 'WX12345', 'T M B 123');
    } catch (\Ingreen\DaktelaPolicy\Support\AppException $exception) {
        assertSameValue(502, $exception->statusCode());
        assertSameValue('invalid_daktela_response', $exception->errorCode());
        assertSameValue('/api/v6/crmRecords', $exception->details()['path']);
...
test('Daktela module creates policy CRM record when no matching record exists', function (): void {
    ...
    $response = $module->saveConfirmedPolicyData('123', ExtractedPolicyData::fromFields([
        ...
        'cena_pakietu' => '3 200 PLN',
        'cena_wznowienia' => '3 400 PLN',
        'oc_cena' => '100 PLN',
        'ac_cena' => '2 100,50 PLN',
        'cena_nnw' => '50 PLN',
        'cena_assistance' => '80 PLN',
        'gap_cena' => '900 PLN',
        'cena_przedluzonej_gwarancji' => '1 200 PLN',
    ], '{}'), testPolicyPdf());
    parse_str((string) $fake->requests[3]['body'], $body);
    ...
    assertSameValue('POST', $fake->requests[3]['method']);
    assertSameValue('https://daktela.example/api/v6/crmRecords.json', $fake->requests[3]['url']);
    assertSameValue('application/x-www-form-urlencoded', $fake->requests[3]['headers']['Content-Type']);
    ...
    assertSameValue('3200', $body['customFields']['cena_pakietu']);
    assertSameValue('3400', $body['customFields']['cena_wznowienia']);
    assertSameValue('100', $body['customFields']['oc_cena']);
    assertSameValue('2100.5', $body['customFields']['ac_cena']);
    assertSameValue('50', $body['customFields']['cena_nnw']);
    assertSameValue('80', $body['customFields']['cena_assistance']);
    assertSameValue('900', $body['customFields']['gap_cena']);
    assertSameValue('1200', $body['customFields']['cena_przedluzonej_gwarancji']);
...
test('Daktela module updates policy CRM record when exactly one matching record exists', function (): void {
    $fake = new FakeDaktela([
        ...
        '/api/v6/crmRecords/record_policy/attachments.json' => existingTestPolicyAttachmentResponse(),
    ...
    assertSameValue('PUT', $fake->requests[3]['method']);
    assertSameValue('https://daktela.example/api/v6/crmRecords/record_policy.json', $fake->requests[3]['url']);
...
test('Daktela module creates vehicle CRM record when no matching vehicle record exists', function (): void {
    ...
    assertSameValue('POST', $fake->requests[5]['method']);
    assertSameValue('https://daktela.example/api/v6/crmRecords.json', $fake->requests[5]['url']);
...
test('Daktela module updates vehicle CRM record when exactly one matching vehicle record exists', function (): void {
    ...
    $module->saveConfirmedPolicyData('123', ExtractedPolicyData::fromFields([
        ...
        'rocznik' => '2 025',
        'przebieg' => '98 765 km',
        'wartosc_pojazdu_brutto' => '150 000 PLN',
    ], '{}'), testPolicyPdf());
    parse_str((string) $fake->requests[6]['body'], $body);
    ...
    assertSameValue('98765', $body['customFields']['przebieg']);
    assertSameValue('150000', $body['customFields']['wartosc_pojazdu_brutto']);
});
...
test('Daktela module aborts vehicle CRM save when more than one matching vehicle record exists', function (): void {
    ...
    $module->saveConfirmedPolicyData('123', ExtractedPolicyData::fromFields([
        ...
        assertSameValue(['PUT', 'GET', 'GET', 'GET', 'PUT', 'GET'], array_map(
            static fn (array $request): string => $request['method'],
            $fake->requests
        ));
        return;
